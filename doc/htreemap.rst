...
great performance with large keys. It also offers entry expiration based on size or TTL.
...
new bigger table. HTreeMap uses auto-expanding 4 level Hash Tree, so it never needs resizing.
Hash Tree also saves lot of space compared to traditional hash tables. 
...
very bad for persistence, it can lead to data loss.
...
for keys. For example ``Serializer.BYTE_ARRAY`` uses TODO methods from Arrays
...
`HTreeMap` has major advantage `BTreeMap` with large keys. Unlike
BTreeMap it only stores hash codes in tree nodes. Each lookup on
BTreeMap deserializes tree nodes together with their keys. 
