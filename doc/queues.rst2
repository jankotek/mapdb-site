...
Lock-free queues in MapDB have several limitations, for example it is
not possible to count elements without actually removing them. Queues in
MapDB usually implement only minimal subset from ``BlockingQueue``
interface. In future we will introduce ``List`` which will fully
implement ``BlockingDequeue`` and ``List`` interfaces, this will use
...
This only takes two extra parameters. Fist you can supply custom
serializers used on entries.

Second param decides lock based eviction. Lock-free (false) means better
...
Second parameter is queue size. If number of entries exceeds the queue
size, some entries will get overwritten and lost.
