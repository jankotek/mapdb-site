...
I have some experience when it comes to memory map (mmap) files and database storag.e 
...
And if you google mmap related issue in Java, MapDB related bug report will probably one of the first results.
...
Lets start with common issues memory mapped (mmap) files have. 
...
Usually your code will fail with exception bellow. 
...
There is no way to close memory mapped buffer, 
mmap file handles are only released once `ByteBuffer` gets garbage collected.
...
However the `ByteBuffer` can not be used after it was unmapped, any access is  invalid memory operation and causes JVM to crash. 
...
If file region was mmaped on Windows, it is not possible to delete that file after, even after it was closed. 
File remains undeletable, even after it was correctly closed, unmapped and JVM process exits. 
One can only delete that file after restart operating system restart. 
...
There is long bug report [opened for MapDB](https://github.com/jankotek/mapdb/issues/442). 
I solved it by preallocating space, when file expands it is filled with `FileOutputStream` to ensure all file space 
is allocated. 
However this slows down MapDB inserts significantly and I need better method.
...
Also it makes it slow to handle large files. It takes a long time to mmap a few thousands new `ByteBuffer` (1MB regions and 20GB file) when file is open. 
...
Files are mmaped in read-only mode
It uses `FileChannel` like method to write changes to disk. 
...
That is important to prevent data corruption. In fact LMDB does not have to use write-ahead-log exactly for this reason. 
...
So I think I found new way to write append-only files such as Write Ahead Log. 

## Truncated mmap files

Here is the solution to another problem: How do you memory map growing file? 
