...
Most DBs engines juggle with byte arrays and leave serialization and
...
-  Outstanding performance comparable to low-level native DBs
   (TokyoCabinet, LevelDB)
...
-  Very flexible; works equally well on an Android phone and a
   supercomputer with multi-terabyte storage.

-  Modular & extensible design; most features (compression, cache, async
   writes) are just ``Engine`` wrappers. Introducing new functionality
   (such as network replication) is very easy.

-  Highly embeddable; 200 KB no-deps jar (50KB packed), pure java, low
   memory & CPU overhead.

-  Transparent encryption, compression, checksums and other optional
   stuff.

Caching
-------

MapDB provides instance cache to minimize deserialization overhead.
Instance cache is tightly integrated with storage engine and is one of
the reasons why MapDB is so fast.

MapDB does not use fixed size disk pages. It also does not have settings
...
This is fixed size random removal cache. It has very small overhead and
is very fast. It uses fixed size hash table and entries are removed
randomly on hash collisions. Is activated by default with size 32768.

LRU instance cache
~~~~~~~~~~~~~~~~~~

Fixed size cache with Least Recently Used entry removal. It uses Cleanup
...
Unbounded instance cache with entries removed after GC collection. It
...
Unbounded instance cache with all entries removed when heap memory runs
...
activated by DBMaker.cacheHardRefEnable()

Serialization
-------------

MapDB has its own space efficient serialization tightly optimized for
minimal CPU overhead. In many cases it outperforms specialized
serialization frameworks such as Kryo or Protobufs.

MapDB serialization tries to mimic Standard Java Serialization behavior.
So you may use Serializable marker interface, customize binary format
with Externalizable methods and so on.

MapDB serialization framework is completely transparent and activated by
...
implementing
`Serializer <http://www.mapdb.org/apidocs/org/mapdb/Serializer.html>`__.

Hand Coded Serializers
~~~~~~~~~~~~~~~~~~~~~~

MapDB hand coded serializers for most classes in ``java.lang`` and
``java.util`` packages. Custom serializers have maximal space efficiency
and very low CPU overhead. For example:

-  ``j.l.Date`` requires only 9 bytes: 1-byte-long serialization header
   and 8-byte-long timestamp.

-  For ``true``, ``String("")`` or ``Long(3)`` MapDB writes only
   single-byte serialization header.

-  For array list and other collections MapDB writes serialization
   header, packed size and than data itself.

Generic POJO serialization
~~~~~~~~~~~~~~~~~~~~~~~~~~

For other classes MapDB uses generic POJO serialization. It traverses
...
larger number of records. For example take following class:

::

     class Person{ int age = 40; String nickname = "Agent Smith"; }

Standard Java Serialization stores this class in 104 bytes with many
redundant information such as field names:
...
MapDB performs serialization and storage modifications in background
...
MapDB has many other small features which would not fit on this page.
You may find more by reading `DBMaker
documentation <http://www.mapdb.org/apidocs/org/mapdb/DBMaker.html>`__
or MapDB source codes.

Transparent compression
~~~~~~~~~~~~~~~~~~~~~~~

MapDB supports transparent compression using `LZF
compressor <http://oldhome.schmorp.de/marc/liblzf.html>`__. It is very
...
be enabled by DBMaker.compressionEnable()

Transparent encryption
~~~~~~~~~~~~~~~~~~~~~~

MapDB allows storage encryption using `XTea
algorithm <http://en.wikipedia.org/wiki/XTEA>`__. It is fast and sound
...
DBMaker.encryptionEnable(String password)
